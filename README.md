...
- 로그인·PC 등록 화면과 명령 입력창 제공
- `/` 슬래시 명령 자동완성 트리와 자연어 입력을 한 입력창에서 처리
- **WebGPU가 있으면 `/요약`을 브라우저 안에서 직접 수행** — 원문을 서버에 보내지 않음
- 작업 진행 상태·결과 카드·요청 이력 표시

실행 위치는 서버가 정합니다. 이 저장소는 그 결정을 따르되, `/요약`만 예외적으로
브라우저가 스스로 판단해 처리한 뒤 결과를 제출합니다.
