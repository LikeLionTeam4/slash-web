...
export type CommandNode = {
   ...
   * 값을 받지 않는 명령(`/모델` 같은 패널)은 이 필드가 없다.
   */
  operands?: string[]
  /**
...
// redundant everywhere it appeared. /파일/검색 → /파일 (파일's only other action, 휴지통, stayed an
// explicit child until it was hidden 2026-08-31 — see git history — since it was never wired to a
// backend). /검색/네이버 and /지도·/길찾기 under 네이버 collapsed into one 네이버 node —
