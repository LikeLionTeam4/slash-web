...
- Nav: icon+label rows (히스토리 / 대시보드 / 즐겨찾기 / 일정) — only 히스토리 and 대시보드 route anywhere; 즐겨찾기/일정 are named but inert, same as the still-placeholder settings categories (§4 Settings). **파일 was removed from here (2026-07-31)** — it duplicated the now-real 설정 > 파일 category with no distinct destination of its own (this inert copy never routed anywhere, unlike 히스토리/대시보드). Muted by default, foreground on hover, no gradient here (gradient stays reserved for the mark/focus-ring/wash per §2).
...
- Two-pane layout: a left category list (일반/파일/계정/개인정보 보호/결제/사용량/연동 — icon+label, active state = `foreground/10` bg) and a right content pane for the active category.
- 일반 (General) and 파일 (Files) are the only fully-built panes for now. 일반: avatar, 성명/닉네임 inputs, a free-text instructions textarea, then a `hairline` divider, then "환경설정" — the **모양** (appearance) control lives here as a 3-icon segmented control (시스템/라이트/다크 — `Monitor`/`Sun`/`Moon`), then **글씨 크기** (보통/크게/매우 크게 — see §3; a matching segmented control, but with word labels since size has no icon that reads unambiguously, and the labels scale with the setting so the choice previews itself), plus a static 검색 결과 글꼴 (result font) dropdown that still does nothing — it now sits next to two live controls, so it reads more like a bug than a placeholder; then a `hairline` divider and **이 기기** (2026-07-31), a read-only list of client-detected values (타임존/운영체제/브라우저/언어/화면 해상도 — `src/lib/clientInfo.ts`) shown for the user's own reference only. These values must never be treated as authoritative by the backend (they're client-reported and trivially spoofable) — if a real decision needs to depend on locale/device, the backend should derive it independently from request headers (`Accept-Language`, `User-Agent`) or server-side lookup, not trust this display. 파일: just the folder-management section described below (§4 File search) — it has no separate section heading of its own beyond "파일 검색 폴더," since the category label already says so. Other categories render a plain "준비 중이에요" placeholder — present in the nav (for structural fidelity) but intentionally not fleshed out yet.
...
**File search (`/파일`)**
- **Where to search is a setting, not part of the search** (2026-07-31, moved to its own category later the same day). Folders are added once in 설정 > 파일 (add / remove / 재연결, plus the read-only top-level folder escape hatch and the Chrome-blocks-sensitive-folders warning). The search panel itself only searches. Picking folders inline meant every search asked for two decisions — where, then what — and the rarely-changing one was in the way of the frequent one.
...
**Revised again:** 2026-07-31 (later still) — 설정 > 일반 gained **이 기기**, a read-only list of client-detected values (타임존/운영체제/브라우저/언어/화면 해상도, `src/lib/clientInfo.ts`) for the user's own reference (§4 Settings). These are display-only by design — spoofable client JS values, never a source of truth for backend decisions; a real locale/device-dependent decision still has to come from request headers or server-side lookup. Same session: 파일 검색 폴더 (previously a subsection of 일반, moved there earlier the same day) got promoted to its own top-level category, **파일**, alongside 일반/계정/개인정보 보호/결제/사용량/연동 — it's a distinct enough domain (folder permission management) that nesting it under 일반 buried it, and now it's the second category with real content instead of a "준비 중이에요" placeholder. The sidebar's own long-inert 파일 nav row (§4 Sidebar) was removed in the same pass — it never routed anywhere and now just duplicates the real settings category with no destination of its own.
