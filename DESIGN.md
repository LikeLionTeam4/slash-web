...
- **IME-safe Enter (2026-07-30 fix)**: the input tracks Hangul/Japanese/Chinese composition via `onCompositionStart`/`onCompositionEnd` (a ref, not state — no need to re-render). Any Enter handling in `handleInputKeyDown` bails out while `isComposing` is true, *and* for a short (~50ms) grace window right after composition ends, since some browsers still report `isComposing: false` on the very keydown that confirms the last syllable. Without this, pressing Enter right as a word like "검색" finishes composing double-typed its last syllable — Enter was simultaneously confirming the IME composition and triggering our own selection/submit logic. Don't remove this guard when touching Enter handling.
